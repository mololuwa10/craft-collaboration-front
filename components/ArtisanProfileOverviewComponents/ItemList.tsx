...
import { ArtisanProfile, fetchProductsByArtisanId } from "@/lib/dbModels";
import { Heart, Star } from "lucide-react";
import Link from "next/link";

const ItemList = ({ artisan }: { artisan: ArtisanProfile }) => {
	interface Product {
		productId: number;
		productName: string;
	...
	useEffect(() => {
		const fetchAndSetProducts = async () => {
			try {
				const products = await fetchProductsByArtisanId(artisan.artisanId);
